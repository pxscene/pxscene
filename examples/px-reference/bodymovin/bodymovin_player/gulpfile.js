...
var index_html = './player/index.html';
        ...
        .pipe(cheerio(function ($, file) {
            $('script').each(function (index, elem) {
                if(elem.attribs.src){
                    ...
                    if(elem.attribs['renderer'])
                    {
                        exclude = true;
                        for(i=0;i<len;i+=1)
                        {
                            if(elem.attribs['renderer'] == excluded[i]){
                                exclude = false;
                                break;
                            }                            
                        }
//                        console.log("GULP >>  " + elem.attribs.src + " >> " + (exclude ? "SKIP" : "INCLUDE") + " > RENDERER: " + elem.attribs['renderer'] );                                
                    }
...
gulp.task('buildSourcesPx', function() {
    return buildVersion(['data-skip', 'pxscene']);
});
...
gulp.task('buildPxFull', ['buildSourcesPx'], function() {
    return gulp.src(srcs)
        .pipe(concat('bodymovin.'+px_version+'.js'))
        .pipe(wrap(moduleWrap))
        .pipe(wrap(pxSceneWrap))
        .pipe(gap.prependFile('./player/pxscene/pxstubs.js'))
        .pipe(gulp.dest('build/player/'));
});

gulp.task('buildPxFullMin', ['buildSourcesPx'], function() {
    return gulp.src(srcs)
        .pipe(concat('bodymovin.'+px_version+'.min.js'))
        .pipe(wrap(moduleWrap))
        .pipe(wrap(pxSceneWrap))
        .pipe(gap.prependFile('./player/pxscene/pxstubs.js'))
        .pipe(uglify(uglifyOptions))
...
gulp.task('default', [ 'buildPxFull', 'buildPxFullMin' ]);