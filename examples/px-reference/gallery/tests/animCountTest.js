...
var container = scene.create({t:"image",parent:root,w:600});
    ...
    var line = scene.create({t:"image",url:baseURL+"images/blurredline.png",x:x,y:y+50,
                                  parent:container});
    var text = scene.create({t:"text",text:"TWEEN_LINEAR: Oscillate 5 times",
    ...
    image.animateTo({x:550},1,scene.animation.TWEEN_LINEAR, scene.animation.OPTION_OSCILLATE, 5).then(function() {
...
updateSize(scene.getWidth(), scene.getHeight());
  ...
  console.error("Import failed for animCountTest.js: " + err)
});
