...
px.import({ scene: "px:scene.1.js" }).then(function ready(imports) {
  ...
  var screensaverPromise = px.getModuleFile('https://github.com/jaga456456/pxscene-1/blob/gh-pages/examples/px-reference/polaroid/testimages.json');
  ...
  function recyclePictures() {
    ...
    item.animateTo({
      x: randomInt(50 + sidePadding, scene.w - (polaroidW) - 50),
      y: randomInt(targetY(), scene.h - (polaroidH) - 25),
      r: (doRotation == 1) ? randomIntFromList([-15, 11]) : 0,
      sx: 1, sy: 1, a: 1
    }, 2.5, scene.animation.TWEEN_STOP, scene.animation.OPTION_LOOP, 1)
      .then(function (savedPic) {
        ...
        if (pictures.numChildren > numVisible - 1) {
          ...
          f.animateTo({ a: 0 }, 0.60, scene.animation.TWEEN_LINEAR, scene.animation.OPTION_LOOP, 1)
            .then(function (f) {
              f.remove();
            });
        }
  ...
  function newPicture() {
    ...
    if (reusePictures == true) {
      ...
      var picture = scene.create({
        t: "object", parent: picturesFg,
      ...
      fg.ready.then(function (pic) {
        ...
        // end scale and crop
        picture.animateTo({
          x: randomInt(50 + sidePadding, scene.w - (polaroidW * fg.sx) - 50),
          y: randomInt(targetY(), scene.h - (polaroidH * fg.sx) - 25),
          r: (doRotation == 1) ? randomIntFromList([-15, 11]) : 0, sx: 1, sy: 1
  ...
  function firstNewPicture() {
    ...
    firstPicture.animateTo({
      x: randomInt(50 + sidePadding, scene.w - (polaroidW * firstFg.sx) - 50),
  ...
  var putUpError = function (isError, errorCode) {
    ...
    var postit = scene.create({
      t: "image", parent: root, x: 1200, y: 1200, w: 500, h: 500, url: basePackageUri + "/images/post-it.png",
      stretchX: scene.stretch.STRETCH, stretchY: scene.stretch.STRETCH,
      a: 0
    });
    var textBox = scene.create({
      t: "textBox", parent: postit, x: 50, y: 25, w: 400, h: 400, textColor: 0x000000FF,
      a: 1, r: -2.6,
      font: captionFont,
    ...
    var errorCodeTextBox = scene.create({
      t: "textBox", parent: postit, x: 50, y: 425, w: 400, h: 60, textColor: 0x000000FF,
      a: 0, r: -2.6,
      font: captionFont,
