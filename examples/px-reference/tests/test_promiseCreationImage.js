...
var image = scene.create({t:"image",parent:root, x:0,y:100, url:badImageUrl});

var beforeStart = function() {
  // Ensure rejected promise on first, invalid url 
  // before beginning the rest of the test
  console.log("test_promiseCreationImage start.....");
...
var tests = {
  ...
  testResetToGoodImageUrl: function() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      image.ready.then(function(o) {
        results.push(assert(true, "testResetToGoodImageUrl: image promise resolution received"));
      }, function rejection(o) {
        console.log("testResetToGoodImageUrl: image rejection received");
        results.push(assert(false, "testResetToGoodImageUrl: image promise resolution expected, but rejection was received"));
      }).then( function(obj) {
        resolve(results);
      });
    });
...
if(manualTest === true) {
  ...
  console.error("Import failed for test_promiseCreationImage.js: " + err)
});
