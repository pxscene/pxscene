...
px.import("px:scene.1.js").then( function ready(scene) {
  ...
  for(var i = 0; i <= min(100,len); i++) {
    ...
    var t = scene.create({t:"rect",y:top,parent:cf,w:w,h:h,r:45,cx:w/2,cy:h/2,fillColor:0x00000070});
    // tile image
    var ti = scene.create({t:"image",url:url,parent:t,w:w,h:h,cx:w/2,cy:h/2, id:"image"});
    // reflection
    var tr = scene.create({t:"image",parent:ti,w:w,h:h,cx:w/2,cy:h,r:180,sx:1.0,sy:0.5,});
  ...
  function updateSelection(oldSelection, newSelection)
  {
    ...
    for (var i = 0; i < numTiles;i++)
    {
      ...
      else if (i < newSelection)
      {
        ...
        {
          o.animateTo({r:-45,sx:1,sy:1,x:x},0.8,tween,
                      scene.animation.LOOP,1);
        }
        x+= wspace;
      }
