...
px.import("px:scene.1.js").then( function ready(scene) {
  ...
  // Detail widgets
  var dtl = scene.create({t:"rect",parent:root, y:0,w:0, h:0, fillColor:0x121212d0,
  ...
  for(var i = 0; i <= min(100,progs.length); i++) {
    ...
    // tile image
    var ti = scene.create({t:"image",url:url,parent:t,w:w,h:h,cx:w/2,cy:h/2, id:"image"});
    // reflection
    var tr = scene.create({t:"image",parent:ti,w:w,h:h/2,cx:w/2,cy:h,rx:1,rz:0,sx:1.0,sy:0.3,r:180, id:"tr"}); 
  ...
  function showDetail(detail, x) {
    ...
    var newX=x-(444/2);  
    dtlVisible = true;             
    dtl.animateTo({a:1,x:150,y:200,w:1000, h:400},0.4,scene.animation.TWEEN_EXP_2,scene.animation.LOOP,1);
  ...
  cf.on("onKeyDown", function(e){
    ...
    if( e.keyCode == 37 || e.keyCode == 39) {
        ...
        if (oldSelection != newSelection) {
          ...
          if(dtlVisible === true) {
            dtlVisible = false;
            dtl.animateTo({a:0,y:0,w:0,h:0},0.3,scene.animation.TWEEN_EXP_2,scene.animation.LOOP,1);   
            dtlImage.url = "";
            dtlTitle.text = "";
            dtlDesc.text = "";
            dtlYear.text = "";  
